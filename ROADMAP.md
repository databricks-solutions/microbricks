...
- [ ] `pr-validate.yml` `bundle-validate` and `preview-deploy` jobs reach Databricks — *blocked: GH-hosted runner egress IPs are not on the dev workspace's FE-VM-managed allowlist (see `github-runner-ip-acl` memory note). Decision: ship Phase 7 with these jobs red; Phase 8 picks an unblock option.*
- [ ] Close PR #1 without merging. `pr-cleanup.yml` runs. *(deferred — same IP-ACL blocker; the cleanup workflow has the same auth path)*
- [ ] Push to `develop`. `deploy-dev.yml` deploys all six services + portal to dev. *(deferred — same blocker)*
- [ ] Tag `v0.0.1`. `deploy-prod.yml` waits for approval, then deploys. *(prod has no IP ACL, so this is the first env that would actually work end-to-end through CI; covered in Phase 8 once the dev unblock is in place)*
...
0. **Unblock CI → Databricks** (carried over from Phase 7). Pick one of the three options in the `github-runner-ip-acl` memory note:
   - self-hosted GH runner inside FE-VM (cleanest, heaviest);
   - separate non-managed IP ACL containing GitHub's `actions` IP ranges (lightest, requires periodic refresh);
   - disable IP ACL on `dev` only (acceptable for a reference-only workspace).
   Until this is done, `pr-validate.yml` `bundle-validate` + `preview-deploy`, `pr-cleanup.yml`, `deploy-dev.yml`, `deploy-test.yml`, and `nightly-orphan-cleanup.yml` cannot reach Databricks. `deploy-prod.yml` works without intervention because prod has no IP ACL.
...
*Last updated: 2026-06-05 — Phase 7 landed on the same DAB-bundle branch (six GitHub Actions workflows + three Lakebase-branch helpers + bundle `app_name_suffix` variable + `pythonpath = ["src"]` fixes across all seven Python projects + db.py type fixes propagated across all six services). PR #1 against `develop` exercised the workflows live: non-Databricks jobs go green, Databricks jobs are blocked by FE-VM's managed IP ACL. Phase 8 picks an unblock option.*
