...
System of record for scheduled and historical visits, open slots, and visit types.

**Owns:** `appointment`, `appointment_slot`, `visit_type`
**References (by ID, no FK):** `patient_id` (patient_db), `provider_id` (provider_db)
**Lakebase project:** `projects/appointment-{dev,test,prod}`
...
databricks postgres create-branch projects/appointment-dev "feat-$SLUG" \
  --json '{"spec":{"source_branch":"projects/appointment-dev/branches/production"}}' \
  -p hc-dev
...
- [Data model](../../HEALTHCARE_DATA_MODEL.md#3-appointment-svc--appointment_db)
...
└── routers/
    └── appointments.py    # GET /appointments, GET /appointments/{id}

migrations/
├── env.py                 # Alembic env wired to OBO + Lakebase
└── versions/
    └── 0001_initial_appointment_schema.py
...
seed/
└── seed.py                # Idempotent synthetic-data loader (~200 appointments).
```

## Seeding

After `alembic upgrade head` against your dev project's `production` branch
(and after seeding `patient-svc` + `provider-svc` so referenced IDs make
chronological sense):

```bash
make seed-appointment                # this service only
# or
make seed-dev                        # all six services in dependency order
```

`seed/seed.py` populates `visit_type` and `appointment` rows with deterministic
`patient_id` / `provider_id` from `scripts/seeds/_common.py:ID`. Re-running is
