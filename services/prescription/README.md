...
System of record for active and historical prescriptions and the refill workflow.

**Owns:** `medication_catalog`, `prescription`, `refill_request`
**References (by ID, no FK):** `patient_id` (patient_db), `provider_id` (provider_db)
**Lakebase project:** `projects/prescription-{dev,test,prod}`
...
databricks postgres create-branch projects/prescription-dev "feat-$SLUG" \
  --json '{"spec":{"source_branch":"projects/prescription-dev/branches/production"}}' \
  -p hc-dev
...
- [Data model](../../HEALTHCARE_DATA_MODEL.md#5-prescription-svc--prescription_db)
...
└── routers/
    └── prescriptions.py   # GET /prescriptions, GET /prescriptions/{id}

migrations/
├── env.py                 # Alembic env wired to OBO + Lakebase
└── versions/
    └── 0001_initial_prescription_schema.py
...
seed/
└── seed.py                # Idempotent synthetic-data loader (~30 prescriptions, 10 refills).
```
...
```bash
make seed-prescription               # this service only
# or
make seed-dev                        # all six services in dependency order
```

`seed/seed.py` populates `medication_catalog`, `prescription`, and
`refill_request` with deterministic IDs that reference the same patients and
providers as the other seeds. Re-running is safe (`ON CONFLICT (id) DO
NOTHING`).
